...
The above steps are similarly implemented to November data as well, as the data is read and converted into readable form, the next step is sorting the data based on fire and non-fire dates. 
...
Since TROPOMI data is obtained from the NASA-TROPOMI satellite, since satellite is not effective in capturing data at all locations. And as GEOS-CF is a model, it is continuous and has data values at all locations. As both the data sets are differing in terms of locations, it wouldn't be appropriate to compare them. Therefore, it would be appropriate when the GEOS-CF data are interpolated at TROPOMI locations. The code shown below does the following. 
...
```
The above steps could be similarly repeated for non-fire dates in the month of August, and fire and non-fire dates in November month as well. 
...
```{r, message=F, warning=F}
trop_plot_aug_8 <- ggplot(fire_date_plot_aug_8,aes(x=Longitude,y=Latitude, color= NO2_Concentration))+
  geom_point()+
  xlab("Longitude")+
  ylab("Latitude")+
  labs(color='NO2 Concentration in molecules/m2')+
...
## Regression, Bias and Error Analysis 
...
bias <- c(14.87, 6.62, 0.68, 1.11, 0.15, 0.81, 0.17, 0.806)
...
r2_plot <- ggplot(df, aes(x=x_data, y= y_data)) + 
  ...
  xlab("Days")+
  ylab("r2")+
  ggtitle("r2 values for fire and non-fire dates")

bias_plot <- ggplot(df, aes(x=x_data, y= bias)) + 
  geom_point()+
  xlab("Days")+
  ylab("Bias")+
  ggtitle("Bias values for fire and non-fire dates")

geos_conc_plot <- ggplot(df, aes(x=x_data, y= geos_conc)) + 
  geom_point()+
   xlab("Days")+
  ylab("NO~2~ Concentration for GEOS interpolated TROPOMI data in molecules/m2")+
  ggtitle("GEOS-Interpolated TROPOMI for fire and non-fire dates")

trop_conc_plot <- ggplot(df, aes(x=x_data, y= trop_conc)) + 
  geom_point()+
  xlab("Days")+
  ylab("NO~2~ Concentration for TROPOMI data in molecules/m2")+
  ggtitle("TROPOMI for fire and non-fire dates")

reg_plot <- plot(res_data_nov_22,main = "TROPOMI" ,ylim=c(-5e+15 ,3e+15), ylab = 'e (Residuals)', xlab ='Predicted Values')
...
From the bias plot we can also say that TROPOMI and GEOS-Interpolated TROPOMI NO~2~ data correlated to a grater extent for days which didn't have fire counts as compared with the days that had fire counts. 

The plots between residuals and predicted values can be analyzed for heterostedaticity, whenever their is inconsistency in predicting the values according to the model, i.e., whenever the model serves to be accurate for lower values and fails for higher values, it would be accounted for a case of heterostedaticity. In this case the model could not longer be considered as accurate. 

The above statements could lead us to a conclusion that satellite and TROPOMI data correlated well for non-fire days as compared to the days which had fire counts. 


# References 

1. https://disc.gsfc.nasa.gov/
2. https://portal.nccs.nasa.gov/datashare/gmao/geos-cf/v1/das/
